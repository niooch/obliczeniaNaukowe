...
% Węzły równoodległe na przedziale [-1,1]:
\[
...
\]
gdzie \(\ell_k(x)\) to wielomiany Lagrange'a:
\[
\ell_k(x) = \prod_{\substack{j=0 \\ j \neq k}}^{n}
\frac{x - x_j}{x_k - x_j}.
\]

% Klasyczny wzór na błąd interpolacji:
\[
...
% Istota zjawiska Rungego (brak zbieżności w normie maksymalnej):
\[
\lim_{n \to \infty} \max_{x \in [-1,1]} |f(x) - p_n(x)| \neq 0,
\]
a wręcz
\[
\limsup_{n \to \infty} \max_{x \in [-1,1]} |f(x) - p_n(x)| > 0.
\]

% Stała Lebesgue'a dla węzłów równoodległych (rosnąca bardzo szybko z n):
\[
\Lambda_n = \max_{x \in [-1,1]} 
\sum_{k=0}^{n} |\ell_k(x)|,
\quad \Lambda_n \xrightarrow[n\to\infty]{} \infty,
\]
co jest jedną z przyczyn niestabilności interpolacji
w węzłach równoodległych (efekt Rungego).

% Dla porównania – węzły Czebyszewa na [-1,1]:
\[
x_k = \cos\left(\frac{2k+1}{2n+2}\pi\right), \quad k=0,1,\dots,n,
\]
dla których błąd interpolacji dla $f(x) = \frac{1}{1+x^2}$
już \textbf{zbiega} do zera w sensie maksymalnym.
